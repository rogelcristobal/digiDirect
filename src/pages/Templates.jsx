					...
					<PageTitle
						...
						subTitle={
							<Typography variant="subtitle1" className="text-gray-500  ">
								Create a Basic listing template for digiDirect . Copy the raw template
								and paste in Magento
							</Typography>
						}
					/>
				</Box>
						...
						<Box className="w-full box-border space-y-6 ">
							...
							{item.snippet && (
								<Box className=" h-fit w-full box-border rounded-xl  mt-4">
									<CodeSnippetComponent code={item?.snippet}></CodeSnippetComponent>
								</Box>
							)}
						</Box>
					</Box>
                ...
                ${item.reference.viewState ? "text-sky-600" : "text-gray-500"}
                   flex items-center justify-center`}
						>
							<BiChevronRight
								className={`text-xl  ${
									item.reference.viewState ? "text-sky-600 visible" : "invisible"
								}`}
							></BiChevronRight>
							{item.title}
						</Link>
